...
const items: { title: string; subtitle: string; icon: keyof typeof Ionicons.glyphMap; route?: string }[] = [
  ...
  { title: 'SaaS-paketti', subtitle: 'Starter, Pro ja Business -pohja', icon: 'card-outline', route: '/analytics' },
...
export default function MoreScreen() {
  ...
  return (
    <Screen>
      ...
      <Card style={styles.accountCard}><Text style={styles.accountTitle}>{company?.name ?? 'Bidalyx-työtila'}</Text><Text style={styles.accountSubtitle}>{user?.email ?? 'Ei käyttäjää'}</Text><Text style={styles.connected}>Supabase yhdistetty</Text></Card>
...
const styles = StyleSheet.create({ accountCard:{gap:spacing.xs,backgroundColor:colors.black}, accountTitle:{fontSize:typography.h3,fontWeight:'900',color:colors.card}, accountSubtitle:{fontSize:typography.small,color:colors.subtleText,fontWeight:'600'}, item:{flexDirection:'row',alignItems:'center',gap:spacing.md}, iconWrap:{width:44,height:44,borderRadius:22,alignItems:'center',justifyContent:'center',backgroundColor:colors.blueSoft}, textWrap:{flex:1}, title:{fontSize:typography.body,fontWeight:'900',color:colors.text}, subtitle:{marginTop:2,fontSize:typography.small,color:colors.mutedText,fontWeight:'600'}, connected:{marginTop:spacing.xs,fontSize:typography.tiny,color:colors.green,fontWeight:'900'} });
