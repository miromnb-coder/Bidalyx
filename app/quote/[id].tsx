...
export default function QuoteDetailScreen() {
  ...
  if (!quote) {
    return <Screen><Pressable onPress={() => router.back()} style={styles.backButton}><Ionicons name="arrow-back" size={24} color={colors.text} /></Pressable><Text style={styles.pageTitle}>Tarjousta ei löytynyt</Text></Screen>;
  }
  ...
  return (
    <Screen>
      <View style={styles.topBar}><Pressable onPress={() => router.back()} style={styles.backButton}><Ionicons name="arrow-back" size={24} color={colors.text} /></Pressable><StatusBadge status={quote.status} /></View>

      <Card style={styles.heroCard}>
        <View style={styles.heroTop}><View style={styles.heroText}><Text style={styles.kicker}>{quote.serviceLabel}</Text><Text style={styles.heroTitle}>{quote.jobTitle}</Text><Text style={styles.heroSubtitle}>{quote.customerName} · {quote.location}</Text></View><View style={styles.heroIcon}><Ionicons name="document-text-outline" size={28} color={colors.card} /></View></View>
        <View style={styles.divider} />
        <View style={styles.valueRow}><View><Text style={styles.labelLight}>Tarjouksen arvo</Text><Text style={styles.heroPrice}>{formatCurrency(quote.estimatedValue)}</Text></View><View style={styles.validBox}><Text style={styles.validLabel}>Voimassa</Text><Text style={styles.validValue}>{quote.validUntil}</Text></View></View>
      </Card>

      <View style={styles.quickGrid}><Card style={styles.quickCard}><Ionicons name="calendar-outline" size={21} color={colors.blue} /><Text style={styles.quickValue}>{quote.schedule || 'Ei arviota'}</Text><Text style={styles.quickLabel}>Aikataulu</Text></Card><Card style={styles.quickCard}><Ionicons name="images-outline" size={21} color={colors.blue} /><Text style={styles.quickValue}>{attachments.length || quote.imageCount}</Text><Text style={styles.quickLabel}>Kuvaa</Text></Card><Card style={styles.quickCard}><Ionicons name="time-outline" size={21} color={colors.blue} /><Text style={styles.quickValue}>{formatDate(quote.updatedAt)}</Text><Text style={styles.quickLabel}>Päivitetty</Text></Card></View>
...
const styles = StyleSheet.create({
  ...
  heroCard: { backgroundColor: colors.black, gap: spacing.md, padding: spacing.xl, borderRadius: radii.xxl, borderColor: colors.black, ...shadows.hero },
  heroTop: { flexDirection: 'row', justifyContent: 'space-between', gap: spacing.md },
  heroText: { flex: 1 },
  kicker: { fontSize: typography.tiny, color: colors.subtleText, fontWeight: '900', textTransform: 'uppercase', letterSpacing: 0.4 },
  ...
  quickCard: { flex: 1, gap: 4, padding: spacing.md, borderRadius: radii.lg },
  ...
  previewImage: { width: 92, height: 92, borderRadius: radii.lg, backgroundColor: colors.background },
  ...
  statusButton: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: spacing.md, paddingVertical: spacing.sm, borderRadius: radii.full, backgroundColor: colors.backgroundElevated, borderWidth: 1, borderColor: colors.border },
  ...
  linkCard: { flexDirection: 'row', gap: spacing.md, alignItems: 'flex-start', backgroundColor: colors.blueSoft, borderColor: '#DBEAFE' },
